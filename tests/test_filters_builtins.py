...
from chatflow_miner.lib.filters.builtins import AgentFilter, CaseHasActivityFilter, TimeWindowFilter
...
def test_time_window_filter_touches_mode_intersection_behavior():
    ...
    f = TimeWindowFilter(start="2021-01-01T10:30:00", end="2021-01-01T10:45:00", mode="touches")
    mask = f.mask(df)
...
def test_time_window_filter_inside_mode_requires_full_containment():
    ...
    f = TimeWindowFilter(start="2021-01-01T09:30:00", end="2021-01-01T11:30:00", mode="inside")
    mask = f.mask(df)
...
def test_time_window_filter_works_with_missing_end_timestamp_column():
    ...
    f = TimeWindowFilter(start="2021-01-01T09:00:00", end="2021-01-01T11:00:00", mode="touches")
    mask = f.mask(df)
    assert mask.tolist() == [True, False]
